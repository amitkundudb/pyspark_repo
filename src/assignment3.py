...
from pyspark.sql.functions import date_sub, col, date_format, datediff, expr
...
df = df.withColumn("login_date", date_format(col("time_stamp"), "yyyy-MM-dd HH:mm:ss").cast(DateType())).drop(
    "time_stamp")
df.show()
